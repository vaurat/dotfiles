...
# essentiels
sudo apt-get install vim-gtk ssh aptitude dialog backintime-gnome git build-essential subversion ncdu tree goaccess glogg ccze tmux oracle-java8-installer openjdk-8-jdk openjdk-8-source openjdk-11-jdk openjdk-11-source gdebi most httpie curl zsh zsh-doc gufw dconf-tools yarn nodejs nfs-common cifs-utils ant-contrib gnome-boxes python3-pip r-base r-base-dev tilix vim-doc subversion gitg tig zenity gparted most bleachbit htop net-tools
# sudo apt install virtualbox exfat-utils
...
sudo apt-get install kazam calibre gimp gtkhash meld filezilla mysql-workbench gnome-shell-pomodoro shutter treeline gmusicbrowser klavaro focuswriter kmymoney planner meld gtkhash gramps vlc
...
# sudo apt-get install iftop ifstat iptraf wireshark tshark arp-scan netspeed nmap netpipe-tcp chkrootkit rkhunter
